...
def CallConditions(scData, config: ScreenSeqConfiguration):
    ...
    for condition in config.conditionBarcodeMapping.keys():
        ...
        clusterer = MeanShift(bin_seeding=True, min_bin_freq=10, n_jobs=-1).fit(amts.reshape(-1, 1))
    ...
    scData.obs[[x + "_frac" for x in conditionBarcodeFractions.columns]] = conditionBarcodeFractions
    ...
    scData = scData[:, scData.var["feature_types"] != "Custom"]
    return scData
