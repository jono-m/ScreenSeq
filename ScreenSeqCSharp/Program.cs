...
public class ScreenSeq {
    ...
    public static void Main() {
        ...
        var outputPath = "C:/Users/jonoj/Repositories/ScreenSeq/Data/3_8_2023_Validation/ssCounts.csv";
        ...
        for (int laneIndex = 0; laneIndex < r1Files.Length; laneIndex++) {
            ...
            while (NextRead(r1Reader, r2Reader,
                ...
                if (readNumber % 100000 == 0) {
                    Console.WriteLine($"Completed read {{{readNumber}}}. {{{lowQuality}}} low-quality reads detected so far.");
                }
                ...
                if(!cellBcCorrectionMap.ContainsKey(cellBarcode)) {
                    cellBcCorrectionMap[cellBarcode] = GetMostSimilarBarcode(cellBarcode, cellBarcodes);
                }
                if(!ssBcCorrectionMap.ContainsKey(ssBarcode)) {
                    ssBcCorrectionMap[ssBarcode] = GetMostSimilarBarcode(ssBarcode, ssBarcodes);
                }
    ...
    public static string GetMostSimilarBarcode(string inBarcode, string[] barcodes) {
    ...
    public static string[] LoadCellBarcodes(string path) {
        ...
        return dataset.Select(x => x.Substring(0, 16)).ToArray();
    }
